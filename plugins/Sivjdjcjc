...
def generate_html_from_decrypted(decrypted_json_str: str, batch_title: str, batch_thumbnail: str = None) -> str:
    ...
    def generate_chips(lectures):
        ...
        for lec in lectures:
            ...
            chips += f'''
                <a href="{lec["link"]}"{target} class="chip">
    ...
    def generate_section(section_id, title, chips, hidden_class=''):
        return f'''
            <section class="section{hidden_class}" id="{section_id}">
    ...
    for i, teacher in enumerate(sorted_teachers):
        ...
        teacher_sections += generate_section(section_id, teacher, chips, hidden)
    
    month_sections = ''
    for month in sorted_months:
        section_id = f"month-{month}"
        chips = generate_chips(month_groups[month])
        month_sections += generate_section(section_id, month, chips, ' hidden')
