...
<html lang="en">
  <head>
    ...
    <title>{batch_title} — Catalog</title>
    <meta name="description" content="Browse {batch_title.lower()} catalog with light/dark mode support." />
    <style>
      :root {{
        /* Light mode colors */
        --primary: #3b82f6;
      ...
      *, *::before, *::after {{ box-sizing: border-box; }}
      ...
      .theme-toggle:focus-visible {{ outline: 2px solid white; outline-offset: 2px; }}
      ...
      .collapse-btn:focus-visible {{ outline: 2px solid var(--primary); }}
...
{month_sections}
      <footer>
        UI built for educational purposes with light/dark mode support.
      </footer>
    </div>
  </main>
  ...
  <script>
    ...
    tabButtons.forEach(tab => {{
      tab.addEventListener('click', () => {{
        ...
        if (firstSection) {{
          const top = firstSection.getBoundingClientRect().top + window.scrollY - 140;
