...
def generate_html_from_decrypted(decrypted_json_str: str, batch_title: str, batch_thumbnail: str = None) -> str:
...
<body>
    ...
    <main class="container">
        ...
        <div class="search-container">
            <input type="text" class="search-box" id="searchBox" placeholder="🔍 Search lectures by name...">
        </div>
        ...
        <div id="teacher-sections" class="hidden">
            {teacher_sections}
        </div>

        <div id="month-sections" class="hidden">
            {month_sections}
        </div>
    </main>
    ...
    <script>
        ...
        const sections = {{
            'day': document.getElementById('day-sections'),
            'teacher': document.getElementById('teacher-sections'),
            'month': document.getElementById('month-sections')
        }};
        ...
        function filterLectures(searchTerm) {{
            ...
            const allChips = activeSection.querySelectorAll('.chip');
            const allSections = activeSection.querySelectorAll('.section');
            
            let hasVisibleResults = false;

            if (searchTerm === '') {{
                // Show all
                allChips.forEach(chip => chip.classList.remove('search-hidden'));
            ...
            // Hide sections with no visible chips
            allSections.forEach(section => {{
                const visibleChips = section.querySelectorAll('.chip:not(.search-hidden)');
            ...
            if msg.document and msg.document.file_name.endswith('.json'):
                try:
                    ...
                    try:
                        ...
                        batch_thumbnail = item.get('cover_photo') or item.get('thumbnail') or 'https://via.placeholder.com/400x200?text=Catalog'
